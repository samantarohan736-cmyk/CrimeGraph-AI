    ...
    <div className="max-w-4xl mx-auto px-4 py-6 min-h-[calc(100vh-4rem)] flex flex-col justify-between font-sans">
      ...
      <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center gap-2">
          <Link
            to="/network"
            className="text-xs font-medium text-slate-700 dark:text-slate-200 hover:text-black dark:hover:text-white px-2.5 py-1.5 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-[#151D2A] flex items-center gap-1.5 transition-colors"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {EXAMPLE_QUESTIONS.map((item, i) => (
            <button
              ...
              className="text-left p-2.5 rounded-lg bg-white dark:bg-[#151D2A] border border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600 hover:bg-slate-50/80 dark:hover:bg-[#1E293B] transition-all flex items-center justify-between group shadow-sm"
            >
              <div className="min-w-0 pr-2">
                <span className="text-xs font-medium text-slate-800 dark:text-slate-200 block truncate group-hover:text-black dark:group-hover:text-white">
                  {item.query}
                </span>
              </div>
              <ChevronRight className="w-3.5 h-3.5 text-slate-400 group-hover:text-black dark:group-hover:text-white group-hover:translate-x-0.5 transition-transform shrink-0" />
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 py-2 pr-1 mb-4">
        {messages.map((msg, idx) => {
          if (msg.role === 'user') {
            return (
              <div key={idx} className="flex justify-end">
                <div className="max-w-xl px-4 py-2.5 rounded-2xl rounded-br-sm bg-slate-900 dark:bg-brutal-yellow text-white dark:text-black text-sm font-medium leading-relaxed shadow-sm">
                  {msg.content}
                </div>
              </div>
            );
          ...
          return (
            <div key={idx} className="flex justify-start">
              <div className="w-full max-w-3xl p-4 sm:p-5 rounded-2xl rounded-tl-sm bg-white dark:bg-[#151D2A] border border-slate-200 dark:border-[#2A364F] shadow-sm space-y-4">
                {/* Concise Answer */}
                <div className="text-slate-800 dark:text-slate-200 text-sm leading-relaxed font-sans font-normal">
                  {msg.answer}
                </div>

                {/* Connection Path (if applicable) */}
                {showPath && (
                  <div className="p-3 rounded-xl bg-slate-50 dark:bg-[#101725] border border-slate-200 dark:border-slate-800 space-y-2.5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300">
                        <GitCommit className="w-3.5 h-3.5 text-slate-600 dark:text-slate-400" />
                        <span>Identified Connection Path</span>
                      </div>
                      <button
                        onClick={() => navigate(`/network?focus=${msg.supporting_entities[0]?.id}`)}
                        className="text-[11px] font-medium text-blue-600 dark:text-cyan-400 hover:text-blue-800 dark:hover:text-cyan-300 hover:underline flex items-center gap-1"
                      >
                      ...
                      {msg.supporting_entities.map((entity, i) => (
                        <React.Fragment key={entity.id || i}>
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-white dark:bg-[#1E293B] border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white font-medium hover:border-black dark:hover:border-brutal-cyan hover:bg-yellow-50 dark:hover:bg-slate-800 transition-colors shadow-2xs"
                          >
                            <span>{entity.label}</span>
                            <span className="text-[10px] text-slate-600 dark:text-slate-400">({entity.type})</span>
                          </button>
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-3">
                    ...
                    <div className="flex items-center gap-2 flex-wrap text-xs">
                      ...
                      {msg.supporting_entities.map((node) => (
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-[#1E293B] hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 text-xs font-medium transition-colors"
                        >
                  ...
                  {msg.cited_evidence_ids && msg.cited_evidence_ids.length > 0 && (
                    <div className="flex items-center gap-2 flex-wrap text-xs">
                      ...
                      {msg.cited_evidence_ids.map((evId, i) => (
                        <span
                          key={i}
                          className="inline-flex items-center px-2 py-0.5 rounded bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-300 font-mono text-[11px] font-semibold"
                        >
                          {evId}
                        </span>
                      ))}
                  ...
                  {msg.confidence !== undefined && (
                    <div className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-400 bg-slate-50/70 dark:bg-[#101725] p-2.5 rounded-lg border border-slate-100 dark:border-slate-800">
                      <div className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-1.5 py-0.5 rounded shrink-0 border border-emerald-200 dark:border-emerald-800">
                        <CheckCircle2 className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                        <span>{confidencePct}% Confidence</span>
                      </div>
                      <p className="text-xs text-slate-600 dark:text-slate-400 leading-snug">
                        <span className="font-medium text-slate-700 dark:text-slate-300">Reasoning:</span> {reasoningText}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1">
                    ...
                    <Link
                      to="/network"
                      className="font-medium text-slate-700 dark:text-slate-300 hover:text-black dark:hover:text-white flex items-center gap-1 hover:underline"
                    >
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="p-3.5 rounded-xl bg-white dark:bg-[#151D2A] border border-slate-200 dark:border-slate-800 shadow-sm flex items-center gap-2.5 text-xs text-slate-600 dark:text-slate-400">
      ...
      <div className="pt-2">
        ...
        >
          <input
            ...
            className="w-full pl-4 pr-24 py-3 bg-white dark:bg-[#151D2A] border border-slate-300 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-500 shadow-sm"
          />
          <button
            type="submit"
            disabled={!queryInput.trim() || loading}
            className="absolute right-1.5 px-3.5 py-2 rounded-lg bg-slate-900 dark:bg-brutal-yellow hover:bg-black dark:hover:bg-yellow-300 text-white dark:text-black text-xs font-semibold flex items-center gap-1.5 disabled:opacity-30 disabled:hover:bg-slate-900 dark:disabled:hover:bg-brutal-yellow transition-colors shadow-sm"
          >
            <span>Ask</span>
        ...
        <div className="mt-3 text-center">
          <p className="text-[11px] text-slate-600 dark:text-slate-400 flex items-center justify-center gap-1">
