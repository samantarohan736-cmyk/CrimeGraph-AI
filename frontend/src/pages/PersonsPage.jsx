        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 bg-white border-2 border-black rounded-lg text-xs font-bold text-black focus:outline-none w-56 shadow-brutal-sm"
            />
          </div>

          <div className="flex items-center gap-1 bg-white rounded-lg p-1 text-xs border-2 border-black">
            {['ALL', 'Critical', 'High', 'Medium'].map(r => (
              <button
                key={r}
                onClick={() => setRiskFilter(r)}
                className={`px-2.5 py-1 rounded text-[11px] font-black uppercase transition-all border-2 border-black ${
                  riskFilter === r 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredPersons.map((p, idx) => {
          ...
          return (
            <div
              key={p.person_id}
              className={`p-5 flex flex-col justify-between space-y-4 bg-white ${tiltClass}`}
            >
              <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 
                        ...
                        className="font-black text-sm text-black hover:text-brutal-pink cursor-pointer uppercase font-sans"
                      >
                        {p.name}
                      </h3>
                      <span className="text-[11px] text-slate-700 font-bold">ID: {p.person_id}</span>
                    </div>
                  </div>
                ...
                <div className="p-3 rounded-lg bg-cream-100 border-2 border-black space-y-1.5 text-xs">
                  <div className="flex items-center justify-between text-slate-700">
                    <span className="font-black">ROLE:</span>
                    <span className="text-black font-black">{p.role || 'Syndicate Associate'}</span>
                  </div>
                  <div className="flex items-center justify-between text-slate-700">
                    <span className="font-black">ALIASES:</span>
                    <span className="text-black font-bold truncate max-w-[160px]">{p.aliases || 'None'}</span>
                  </div>
                  <div className="flex items-center justify-between text-slate-700">
                    <span className="font-black">BASE:</span>
                    <span className="text-black font-bold">{p.primary_location}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-3 border-t-2 border-black flex items-center justify-between">
                <button
                  ...
                  className="neo-btn px-2.5 py-1 bg-cream-200 text-black text-xs flex items-center gap-1"
                >
