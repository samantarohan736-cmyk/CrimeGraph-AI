...
export default function GraphLegend({ colorMode = 'type' }) {
  return (
    <div className="absolute bottom-4 left-4 z-20 p-3 rounded-xl bg-white border-[2.5px] border-black shadow-brutal max-w-xs text-xs space-y-2 font-mono">
      <span className="font-black text-black text-[11px] uppercase tracking-wider block">
      ...
      {colorMode === 'type' && (
        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 font-bold text-black">
          {Object.entries(ENTITY_COLORS).slice(0, 6).map(([type, color]) => (
            <div key={type} className="flex items-center gap-2">
              ...
              <span className="text-slate-800 text-[11px] truncate">{type}</span>
            </div>
          ))}
          <div className="flex items-center gap-2 col-span-2 pt-1 border-t-2 border-black">
            <span className="w-3 h-3 rounded-full border-2 border-black bg-brutal-yellow shrink-0" />
            <span className="text-black text-[11px] font-black">Bridge Node (Articulation)</span>
          </div>
        </div>
      )}
      ...
      {colorMode === 'community' && (
        <div className="space-y-1.5 font-bold">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-800 text-[11px]">Cluster 1: Hawala & Wire Flow</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full border-2 border-black" style={{ backgroundColor: COMMUNITY_COLORS[1] }} />
            <span className="text-slate-800 text-[11px]">Cluster 2: Cargo & Contraband</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full border-2 border-black" style={{ backgroundColor: COMMUNITY_COLORS[2] }} />
            <span className="text-slate-800 text-[11px]">Cluster 3: Cyber Extortion</span>
          </div>
        </div>
      )}
