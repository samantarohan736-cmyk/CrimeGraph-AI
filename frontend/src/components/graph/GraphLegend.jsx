...
import { Users, Briefcase, Smartphone, Truck, MapPin, Building, Activity } from 'lucide-react';
...
export default function GraphLegend({ colorMode = 'type' }) {
  return (
    <div className="absolute bottom-4 left-4 z-20 p-3 rounded-xl bg-[var(--bg-secondary)] border-[2.5px] border-[var(--border-color)] shadow-brutal max-w-xs text-xs space-y-2 font-mono transition-colors">
      ...
      {colorMode === 'type' && (
        <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 font-bold text-[var(--text-primary)]">
          ...
          <div className="flex items-center gap-2 col-span-2 pt-1 border-t-2 border-[var(--border-color)]">
            <span className="w-3 h-3 rounded-full border-2 border-[var(--border-color)] bg-brutal-yellow shrink-0" />
            <span className="text-[var(--text-primary)] text-[11px] font-black">Bridge Node (Articulation)</span>
          </div>
        </div>
      )}
