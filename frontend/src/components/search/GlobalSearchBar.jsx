    ...
    <div ref={searchRef} className="relative w-full max-w-lg font-mono">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-9 pr-8 py-2 bg-cream-100 dark:bg-[#1F2937] border-[2.5px] border-black dark:border-slate-700 rounded-lg text-xs font-black text-black dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:bg-white dark:focus:bg-[#111827] shadow-[3px_3px_0_0_#000]"
        />
        {query && (
          <button
      ...
      {isOpen && results && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#111827] border-[3px] border-black dark:border-slate-700 rounded-xl shadow-[8px_8px_0_0_#000000] z-50 max-h-[70vh] overflow-y-auto divide-y-2 divide-cream-200 dark:divide-slate-800">
                ...
                <div className="space-y-1 mt-1">
                  {items.map((item) => (
                    <div
                      ...
                      className="px-3 py-2 rounded-lg hover:bg-cream-100 dark:hover:bg-[#1F2937] cursor-pointer flex items-center justify-between group transition-colors border-2 border-transparent hover:border-black dark:hover:border-slate-600"
                    >
                      <div>
                        <div className="text-xs font-black text-black dark:text-slate-100">
                          {item.title}
                        </div>
          ...
          {Object.values(results).every(arr => arr.length === 0) && (
            <div className="p-6 text-center text-xs font-bold text-slate-700 dark:text-slate-300">
